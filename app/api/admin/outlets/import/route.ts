...
export async function POST(req:Request){
  try{
    ...
    const mapped=rows.map(r=>({
      ...
      venueType: String(r.venueType||r.type||r.Type||"").trim(),
      indoorOutdoor: String(r.indoorOutdoor||r.io||r.IO||"").trim(),
    })).filter(r=>r.name);
    const dir=path.join(process.cwd(),"data"); await mkdir(dir,{recursive:true});
    await writeFile(path.join(dir,"outlets_master.json"), Buffer.from(JSON.stringify(mapped,null,2)));
