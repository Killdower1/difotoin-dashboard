...
export default function LocationReviewDashboard() {
  ...
  // injected fix: load periods
  const [periods, setPeriods] = useState<string[]>([]);
  // injected fix: sort periods desc
  const periodsDesc = (Array.isArray(periods) ? periods : [])
    .filter((p) => /^\d{4}-\d{2}$/.test(p))
    .sort((a, b) => (a < b ? 1 : -1));
  useEffect(() => {
    fetch("/api/periods")
      .then(r => r.json())
      .then(j => setPeriods(Array.isArray(j) ? j : []))
      .catch(() => setPeriods([]));
  }, []);
...
useEffect(() => {
  ...
  const compareOptions = (Array.isArray(periods) ? periods : [])
  .filter((p) => p !== "latest" && p !== period && /^\d{4}-\d{2}$/.test(p))
  .sort((a, b) => (a < b ? 1 : -1));
